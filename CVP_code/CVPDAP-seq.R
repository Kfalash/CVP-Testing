...
DAPseq_counts <- read.table("DAP-seq_family_nbTF_HVG_LVG_random_background.txt",
                            header=T, sep='\t')
...
DAPseq <- read.table("DAPseq_summary.txt",header=T, sep='\t')

DAPseq_nbTF <- dplyr::mutate(DAPseq, nbTF=rowSums(DAPseq[,-1])) %>%
  select(., gene_name, nbTF)

Background <- read.table("All_genes_background.txt", header=TRUE, sep='\t')
